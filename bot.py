...
# PUBLIC_URL: your Railway service's public domain, e.g.
# https://your-service-name.up.railway.app
# Generate this under Railway → your service → Settings → Networking →
# "Generate Domain". Required for the ad-click tracking to work.
PUBLIC_URL = os.environ.get("PUBLIC_URL", "").rstrip("/")
# ----------------------------------------------------------------------------

# Tracks which Telegram user IDs (as strings) clicked through the ad
# redirect link, and WHEN (as a timestamp). Lives in memory only — resets
# on restart, which is fine since it only needs to last one unlock attempt.
_ad_watched = {}

# Minimum seconds that must pass between clicking "Watch Ad" and being
# allowed to tap "I've done both".
MIN_AD_WAIT_SECONDS = int(os.environ.get("MIN_AD_WAIT_SECONDS", "20"))
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not PUBLIC_URL:
        await update.message.reply_text(
            "⚠️ Bot isn't fully configured yet (missing PUBLIC_URL) — please try again later."
        )
        logger.error("PUBLIC_URL is not set — ad tracking link cannot be built.")
        return

    ad_tracking_link = f"{PUBLIC_URL}/ad/{user_id}"

    keyboard = [
        [InlineKeyboardButton("📢 Join Backup Channel", url=channel_invite)],
        [InlineKeyboardButton("🎬 Watch Ad", url=ad_tracking_link)],
    ...
    await update.message.reply_text(
        ...
        "2️⃣ Tap \"Watch Ad\" and view the page that opens\n\n"
        f"Then come back and tap \"I've done both\" (available {MIN_AD_WAIT_SECONDS}s after opening the ad).",
...
async def check_gate(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    ad_click_time = _ad_watched.get(str(user_id))
    if not ad_click_time:
        await query.answer(
            "Please tap \"Watch Ad\" first and view the page that opens.",
            show_alert=True,
        )
        return

    elapsed = time.time() - ad_click_time
    if elapsed < MIN_AD_WAIT_SECONDS:
        remaining = int(MIN_AD_WAIT_SECONDS - elapsed)
        await query.answer(
            f"Please wait {remaining} more second{'s' if remaining != 1 else ''} "
            f"after opening the ad before confirming.",
            show_alert=True,
        )
        return
...
async def ad_redirect(request: web.Request) -> web.Response:
    """Public endpoint the 'Watch Ad' button links to. Records WHEN this
    Telegram user clicked through, then forwards them to the real ad."""
    uid = request.match_info.get("uid", "")
    if uid:
        _ad_watched[uid] = time.time()
        logger.info(f"Ad click tracked for user {uid}")
    raise web.HTTPFound(AD_LINK)


async def _run_web_server_async() -> None:
    app = web.Application()
    app.router.add_get("/ad/{uid}", ad_redirect)
    runner = web.AppRunner(app, handle_signals=False)
    await runner.setup()
    port = int(os.environ.get("PORT", 8080))
    site = web.TCPSite(runner, "0.0.0.0", port)
    await site.start()
    logger.info(f"Ad-tracking web server listening on port {port}")
    # Keep this coroutine alive forever so the server keeps serving
    while True:
        await asyncio.sleep(3600)


def start_web_server() -> None:
    """Runs a tiny web server (in its own thread) just to track ad clicks.
    Railway provides the PORT env var automatically.

    NOTE: we avoid aiohttp's web.run_app() here because it tries to install
    OS signal handlers (SIGINT/SIGTERM), which Python only allows in the
    main thread — since this runs in a background thread, that would crash
    on startup every time. We set up the server manually instead."""
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    try:
        loop.run_until_complete(_run_web_server_async())
    except Exception as e:
