...
# Tracks which Telegram user IDs (as strings) have clicked through the ad
# redirect link. Lives in memory only — resets on restart, which is fine
# since it only needs to last the length of one unlock attempt.
_ad_watched = {}
...
async def check_gate(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not _ad_watched.get(str(user_id)):
        await query.answer(
...
async def ad_redirect(request: web.Request) -> web.Response:
    """Public endpoint the 'Watch Ad' button links to. Marks that this
    Telegram user clicked through, then forwards them to the real ad."""
    uid = request.match_info.get("uid", "")
    if uid:
        _ad_watched[uid] = True
